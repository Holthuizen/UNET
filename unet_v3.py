...
class OxfordPetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_tensor = torch.from_numpy(processed_mask_np).unsqueeze(0)  # (1, H, W)
...
class DiceLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        pred_binary = (pred > 0.55).float().view(-1)
        target = target.view(-1)
        
        intersection = (pred_binary * target).sum()
        dice = (2. * intersection + self.smooth) / (pred_binary.sum() + target.sum() + self.smooth)
...
def save_predictions_as_imgs(predictions,targets, epoch, folder="saved_images/"):
...
def main():
    ...
    optimizer = optim.SGD(model.parameters(), lr=LEARNING_RATE, momentum=0.16)
        ...
        with torch.no_grad():
            ...
            for batch_id, (data, targets) in enumerate(val_loader):
                ...
                if batch_id == (len(val_dataset)//BATCH_SIZE)-1:
                    save_predictions_as_imgs(predictions, targets, epoch, folder="saved_images/")
