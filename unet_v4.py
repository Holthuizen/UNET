...
from torchvision.transforms.v2.functional import to_dtype, resize, normalize, to_image
...
class OxfordPetDataset(Dataset):
    ...
    """
    def __init__(self, root=".", split="trainval", target_size=(512, 512), blur_sigma=1.5):
        ...
        # --- NEW: Define the blur transform for soft masks ---
        if blur_sigma > 0:
            # Calculate kernel size from sigma (a common rule of thumb)
            kernel_size = 2 * int(4.0 * blur_sigma + 0.5) + 1
            self.blur_transform = T.GaussianBlur(kernel_size=(kernel_size, kernel_size), sigma=blur_sigma)
            print(f"Soft masks ENABLED with sigma={blur_sigma}, kernel=({kernel_size}, {kernel_size})")
        else:
            self.blur_transform = nn.Identity() # No-op if sigma is 0
            print("Soft masks DISABLED (blur_sigma=0)")
    ...
    def __getitem__(self, idx):
        ...
        image, mask = self.full_dataset[original_idx] # PIL
        ...
        image_tensor = to_image(image) # (C, H, W) tensor
        image_tensor = to_dtype(image_tensor, dtype=torch.float32, scale=True)

        # --- Process mask (FIXED) ---
        # Convert PIL mask to tensor (1, H, W)
        mask_tensor = to_image(mask) # (1, H, W), dtype=uint8, values [1, 2, 3]
        mask_tensor = to_dtype(mask_tensor, dtype=torch.float32, scale=False)
        
        # Create binary mask: 1.0 where mask is (1 OR 2), 0.0 otherwise
        mask_tensor = ((mask_tensor == 1) | (mask_tensor == 3)).to(torch.float32)

        # --- NEW: Apply Softening (Blur) ---
        mask_tensor = self.blur_transform(mask_tensor)

        # --- Normalize image (define constants locally) ---
...
class TverskyLoss(nn.Module):
    def __init__(self, alpha=1.0, beta=0.8, smooth=1e-6):
        super().__init__()
...
def save_predictions_as_imgs(predictions,targets, epoch, optimizer_name, folder="saved_images/"):
    print(f"Saving prediction images to {folder}...")
    os.makedirs(folder, exist_ok=True)

    predictions_proba = torch.sigmoid(predictions)
    predictions_binary = (predictions_proba > 0.51).float()
    ...
    comparison_grid = torch.cat([targets_rgb, predictions_rgb], dim=0)
    
    vutils.save_image(
        comparison_grid,
        f"{folder}/{optimizer_name}_epoch_{epoch+1}.png",
        nrow=BATCH_SIZE
    )
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        with torch.no_grad():
            total_val_loss = 0
            for batch_id, (data, targets) in enumerate(val_loader):
                data = data.to(DEVICE)
                targets = targets.to(DEVICE)
                predictions = model(data)
                loss = loss_fn(predictions, targets)
                total_val_loss += loss.item()
                #save second-last from batch (last full batch)
                if batch_id == (len(val_dataset)//BATCH_SIZE)-1:
                    save_predictions_as_imgs(predictions, targets, epoch, optimizer.__class__.__name__, folder="saved_images/")
